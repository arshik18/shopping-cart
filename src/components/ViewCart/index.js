...
const ViewCart = (props) => {
  ...
  const renderEmptyCart = () => {
    return <p className={classes.labelCart}>Please add Item to Cart</p>;
  };
  const renderCartItems = () => {
    return cartItemList.map((listItem) => {
      return (
  ...
  const renderPlaceOrderBtn = () => {
    return (
      <Row className={classes.ordersRow}>
        <Col>
          <Button onClick={placeOrder} className={classes.orderBtn}>
            Place Order
          </Button>
        </Col>
      </Row>
      ...
      <Container className={classes.viewCartContainer}>
        <Row className={classes.viewCartRow}>
          <Col lg="7" className={classes.cartItemDisplay}>
            ...
            <hr />
            {cartItemList.length === 0 ? renderEmptyCart() : renderCartItems()}
            {cartItemList.length > 0 && renderPlaceOrderBtn()}
          </Col>
